...
def evaluate(model, graph, features, labels, index, BB_G, bb_features):
     ...
     with torch.no_grad():
        ...
        R = TP/(TP + FN+ e)
        F1 = (2 * P * R)/(P + R +e)
        acc = torch.sum(pred == true).item() * 1.0 / (len(index) + e)
...
def train_model(train_data_node, train_data_edge, BB_info, features, labels):
    ...
    for i in range(len(labels)):
        if labels[i] != -1:
            ...
            test_index.append(i)
    print(len(T_index), len(F_index)) 
    #Training set: 70% instructions
    # T_train = random.sample(T_index, int(len(T_index)*0.74)) 
    # F_train = random.sample(F_index, int(len(F_index)*0.74)) 
    T_train = T_index[0: int(len(T_index)*0.74)]
    F_train = F_index[0: int(len(F_index)*0.74)]
    idx_train = T_train + F_train
    random.shuffle(idx_train)
    idx_train = torch.LongTensor(idx_train)

    #Test Set: 
    idx_val = list((set(T_index) - set(T_train))) + list((set(F_index) - set(F_train)))
    random.shuffle(idx_val)
    
    #Validation Set, 20% of Test set
    idx_test = idx_val[0:int(len(idx_val)*0.2)]
    for i in range(len(idx_test)):
        idx_val.remove(idx_test[i]) #Remove validation set from test set
